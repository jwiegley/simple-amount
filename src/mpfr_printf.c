...
static int stream_out_mpq(mpq_t quant, size_t precision,
                          int zeros_prec /* = -1 */,
                          mpfr_rnd_t rnd /* = GMP_RNDN */, char **bufPtr) {
  ...
  mp_prec_t num_prec = (mpfr_prec_t)mpz_sizeinbase(mpq_numref(quant), 2);
  ...
  mp_prec_t den_prec = (mpfr_prec_t)mpz_sizeinbase(mpq_denref(quant), 2);
  ...
  if (zeros_prec >= 0) {
    ...
    if (point > 0) {
      while (--index >= (point + 1 + (size_t)zeros_prec) && buf[index] == '0')
        buf[index] = '\0';
      if (index >= (point + (size_t)zeros_prec) && buf[index] == '.')
        buf[index] = '\0';
    }
  }
...
void ratio_int64_to_str(signed long num, unsigned long den,
                        unsigned int rnd_mode, size_t prec, char **bufPtr) {
  mpq_t val;
  mpq_init(val);
...
void rational_to_str(char *rational, unsigned int rnd_mode, size_t prec,
                     char **bufPtr) {
  mpq_t val;
  mpq_init(val);
  mpq_set_str(val, rational, 10);
